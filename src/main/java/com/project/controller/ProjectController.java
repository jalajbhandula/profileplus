...
//					System.out.println(fieldName+"Error "+errorMessage);
			...
			//if (file.isEmpty()!=true){
				...
				Path path = Paths.get("D:/SpringWorkspace/WebProject/src/main/resources/static/img/"
...
//		}
		...
		try {
			...
			if (file.isEmpty()) {
				...
				Path existingPath = Paths
						.get("D:/SpringWorkspace/WebProject/src/main/resources/static/img/"
	...
	public String deleteUserInfo(@PathVariable("id") int id) {
		Registration existingUser = projectDAO.findById(id).orElse(new Registration());
		Path existingPath = Paths.get("D:/SpringWorkspace/WebProject/src/main/resources/static/img/"
				+ existingUser.getImageURL());
		try {
			Files.deleteIfExists(existingPath);
		} catch (IOException e) {
